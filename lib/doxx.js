...
class Doxx extends Compiler {
	...
	/**
	 * @description Generates the documentations.
	 */
	generate() {
		...
		var readme = pkg && pkg.readme,
			readMeFile = path.resolve(process.cwd(), this.options.readme || (pkg && pkg.readmeFileName) || 'README.md')
		...
		this.files.forEach(file => {
			...
			var compiled = this.compile(compileOptions);
			mkdirp(this.options.target, err =>{
				if (err) return;
				else 
					fs.writeFileSync(path.join(this.options.target, file.targetName), compiled)
			});
		});
	}
}
