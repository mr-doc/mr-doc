  ...
  generate() {
      ...
      this.files.forEach(file => {
        ...
        if ((folder !== '') && (folders.indexOf(folder) === -1)) {
          folders.push(folder);
          mkdirp.sync(Path.normalize(this.options.output + '/' + folder));
        }
      });
      ...
      (new Theme(this.options)).install().then((result) => {
        ...
        Dir
          .getDirs(this.options.source)
          .filter(folder => this.options.blacklist.indexOf(folder) < 0)
          .forEach(folder => File.ensureDirSync(Path.join(this.options.output, folder)));
