class Symbol {
  ...
  */
  ...
  map(symbol) {
    symbol.tags = this.compact(symbol.tags);
    var tags = {};
    ['type', 'description', 'example', 'file', 'fileoverview',
      'overview', 'param', 'require', 'jsFiddle', 'return'].forEach(tag => {
        tags[tag.toLowerCase() + 's'] = symbol.tags.filter(this.has(tag));
      });
    var { types, descriptions, examples, files, fileoverviews,
      overviews, returns, requires, jsfiddles } = tags;
    if (symbol.tags.length > 0 && symbol.tags.filter(this.has('param')).length > 0) {
