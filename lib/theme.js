...
class Theme {
  ...
  static tasks(options) {
      ...
      let config = {
        src: (() => {
          ...
          else if (options.template.isEnabled()) return process.cwd();
  ...
  static findTheme(options) {
      ...
      log('findTheme - Default:', DEFAULT_THEME,
        'Mr. Doc Path:', mrDocPath,
  ...
  install(options = this.options) {
      ...
      if (options.template.isEnabled() &&
        ...
        log('install - is template:',
          options.template.isEnabled(),
          'is not a starter-kit:', !options.template.isKit());
